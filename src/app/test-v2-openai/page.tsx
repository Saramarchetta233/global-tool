...
import { Upload, FileText, Brain, Download, Play, ChevronLeft, ChevronRight, Sparkles, Zap, Target, Clock, BookOpen, Star, Rocket, Award, MessageCircle, Coins, User, LogOut, History, Calendar } from 'lucide-react';
...
import { saveStudySession, convertResultsToHistory } from '@/lib/study-history';
...
interface StudyResults {
  ...
  domande_probabili?: Array<{ question: string; difficulty: string }>;
...
const processWithAI = async (file: File, language: string, authToken: string, targetLanguage?: string): Promise<StudyResults> => {
  ...
  // Use demo endpoint only in explicit demo mode, otherwise use new OpenAI endpoint
  const isDemoMode = process.env.NEXT_PUBLIC_DEMO_MODE === 'true';
  const endpoint = isDemoMode ? '/api/demo-process-pdf' : '/api/parse-pdf-openai';
  ...
  // Handle the new docContext structure from OpenAI endpoint
  if (data.docContext) {
    console.log('📦 Received docContext from OpenAI endpoint');
    return data.docContext;
  }
  
  // Fallback for demo mode or old structure
  return data;
};
...
const StudiusAIV2: React.FC = () => {
  ...
  const handleSelectFromHistory = (historicalDocument: any) => {
    ...
    const safeResults = {
      ...
      domande_probabili: historicalDocument.domande_probabili || historicalDocument.probableQuestions || [],
      extractedText: historicalDocument.extractedText || '',
      tutorContext: historicalDocument.tutorContext || '',
              ...
              <div className="relative bg-white/10 backdrop-blur-xl rounded-3xl p-8 border border-white/20 shadow-2xl">
                ...
                {file && (
                  <button
                    onClick={async () => {
                      ...
                      try {
                        // Prima prova con il test semplice
                        const simpleResponse = await fetch('/api/simple-pdf-test', {
                          method: 'POST',
                          body: formData
                        });
                        
                        const simpleResult = await simpleResponse.json();
                        console.log('🧪 Simple PDF Test Result:', simpleResult);
                        
                        if (simpleResult.success) {
                          alert(`✅ PDF estratto con successo (Test Semplice)!\nPagine: ${simpleResult.pages}\nCaratteri: ${simpleResult.textLength}\n\nAnteprima:\n${simpleResult.textPreview}`);
                        } else {
                          console.warn('❌ Simple test failed, trying raw extraction...');
                          
                          // Test raw extraction
                          const rawResponse = await fetch('/api/raw-pdf-test', {
                            method: 'POST',
                            body: formData
                          });
                          
                          const rawResult = await rawResponse.json();
                          console.log('🔬 Raw PDF Test Result:', rawResult);
                          
                          if (rawResult.success && rawResult.hasText) {
                            alert(`✅ PDF estratto con metodo RAW!\nMetodo: ${rawResult.method}\nCaratteri: ${rawResult.textLength}\n\nAnteprima:\n${rawResult.textPreview}`);
                            
                            // Test anche l'elaborazione completa con il metodo RAW
                            if (rawResult.textLength > 100) {
                              console.log('🚀 Testing full processing with RAW method...');
                              
                              try {
                                const rawProcessResponse = await fetch('/api/process-pdf-raw', {
                                  method: 'POST',
                                  body: formData
                                });
                                
                                const rawProcessResult = await rawProcessResponse.json();
                                console.log('📚 Raw Process Result:', rawProcessResult);
                                
                                if (rawProcessResult.success) {
                                  alert(`🎉 SUCCESSO COMPLETO!\nRiassunto generato con metodo RAW!\nLunghezza riassunto: ${rawProcessResult.riassunto_breve?.length || 0} caratteri\nFlashcard: ${rawProcessResult.flashcard?.length || 0}\nQuiz: ${rawProcessResult.quiz?.length || 0}`);
                                } else {
                                  alert(`⚠️ Estrazione RAW OK, ma elaborazione AI fallita:\n${rawProcessResult.error}`);
                                }
                              } catch (processError) {
                                console.error('Raw processing failed:', processError);
                                alert('Estrazione RAW OK, ma errore nell\'elaborazione AI');
                              }
                            }
                            
                            return;
                          }
                          // Se fallisce il test semplice, prova quello dettagliato
                          console.warn('❌ Simple test failed, trying detailed debug...');
                          ...
                          console.log('📊 Debug PDF Result:', debugResult);
                          
                          alert(`❌ Estrazione fallita in entrambi i test:\n\nTest Semplice: ${simpleResult.error}\n\nTest Debug: ${debugResult.error || 'Successo parziale'}`);
                        }
                      } catch (error) {
                        console.error('Both tests failed:', error);
                        alert('Errore in tutti i test di estrazione');
                      }
                    }}
                    className="w-full bg-gradient-to-r from-yellow-600 to-orange-600 text-white font-medium py-3 px-6 rounded-xl hover:from-yellow-700 hover:to-orange-700 transition-all duration-300 mb-4"
                  >
                    🔍 Test Estrazione PDF (Multi-Test)
                  </button>
                )}
                ...
                >
                  <div className="flex items-center justify-center gap-3">
                    ...
                    <span>
                      {!user ? '🔒 Accedi per elaborare' : '✨ Genera materiali di studio AI'}
                    </span>
                ...
                {user && (
                  <div className="mt-4 text-center text-sm text-gray-400">
                    <div className="flex items-center justify-center gap-2">
                      ...
                      <span>Costo: 5 crediti per l'elaborazione completa</span>
                    </div>
                  </div>
                )}
            ...
            <div className="relative">
              ...
              <div className="relative bg-white/10 backdrop-blur-xl rounded-3xl p-8 border border-white/20 shadow-2xl">
                ...
                <div className={activeTab === 'tutor_ai' ? 'block' : 'hidden'}>
                  ...
                  <div className="bg-white/5 rounded-2xl p-6 border border-white/10">
                    <TutorChat 
                      docContext={results.tutorContext || results.extractedText || renderContent(results.riassunto_esteso) || renderContent(results.riassunto_breve)}
