...
const advancedTrackingUtils = {
  ...
  trackFacebookPurchase: async (orderData: any, clientEventId: string): Promise<void> => {
    ...
    // CAPI tracking via N8N - SOLO CON DATI COMPLETI
    if (orderData && orderData.imie && orderData.telefon && orderData.adres) {
      // Validazione aggiuntiva per assicurarsi che i campi non siano vuoti
      const isValidData = orderData.imie.trim().length > 0 &&
        orderData.telefon.trim().length > 0 &&
        orderData.adres.trim().length > 0;

      if (!isValidData) {
        console.log('⚠️ Dati incompleti, webhook N8N non inviato:', {
          imie: orderData.imie || 'vuoto',
          telefon: orderData.telefon || 'vuoto',
          adres: orderData.adres || 'vuoto'
        });
        return;
      }

      try {
        console.log(`📡 Sending Purchase to N8N webhook with complete data...`);

        // Hash dei dati sensibili - solo se presenti
        const hashedPhone = await advancedTrackingUtils.hashData(orderData.telefon.replace(/\D/g, ''));
        const hashedFirstName = await advancedTrackingUtils.hashData(orderData.imie.split(' ')[0]);
        const hashedLastName = orderData.imie.split(' ').length > 1 ? await advancedTrackingUtils.hashData(orderData.imie.split(' ').slice(1).join(' ')) : null;
      ...
      console.log('⚠️ Dati ordine mancanti o incompleti - webhook N8N e Telegram bot non verranno triggerati:', {
        hasOrderData: !!orderData,
        hasImie: !!(orderData?.imie),
        hasTelefon: !!(orderData?.telefon),
        hasAdres: !!(orderData?.adres)
      });
    }
  },
  ...
  useEffect(() => {
    ...
    const trackPurchase = async () => {
      try {
        ...
        // Verifica dati prima del tracking - NON usare dati di default vuoti
