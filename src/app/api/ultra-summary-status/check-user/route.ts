...
/**
 * GET /api/ultra-summary-status/check-user
 * Controlla se l'utente ha sessioni con Ultra Summary in elaborazione
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Cerca sessioni dell'utente con ultra_summary_status = 'in_progress'
    ...
    // Filtra solo quelle con ultra_summary_status = 'in_progress'
    const inProgressSessions = sessions?.filter(session => {
      const metadata = session.processing_metadata;
      return metadata && metadata.ultra_summary_status === 'in_progress';
    }) || [];
    ...
    const sessionsWithProgress = inProgressSessions.map(session => ({
      ...
      startedAt: session.processing_metadata?.started_at || session.created_at
    }));
