...
/**
 * POST /api/history/update-ultra
 * Aggiorna la cache Redis con il riassunto_ultra per un documento specifico
 */
export async function POST(request: NextRequest) {
  try {
    ...
    const { sessionId, riassuntoUltra } = await request.json();

    if (!sessionId || !riassuntoUltra) {
      return NextResponse.json({ error: 'sessionId e riassuntoUltra richiesti' }, { status: 400 });
    }
      ...
      if (cachedHistory && Array.isArray(cachedHistory)) {
        ...
        const updatedHistory = cachedHistory.map((doc: any) => {
          if (doc.id === sessionId || doc.sessionId === sessionId) {
            return { ...doc, riassunto_ultra: riassuntoUltra };
          }
          return doc;
        });
        ...
        console.log(`✅ [CACHE_UPDATE] riassunto_ultra aggiunto alla cache per sessione ${sessionId}`);
    ...
    console.error('❌ Errore API update-ultra:', error);
